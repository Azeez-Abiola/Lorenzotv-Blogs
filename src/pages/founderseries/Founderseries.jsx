...
import { FounderSeriesSkeleton } from "../../components/Skeletons/Skeletons"
...
function Founderseries() {
  ...
  const loadRelatedPosts = useCallback(() => {
    ...
    const url = `${import.meta.env.VITE_API_BASE_URL}/blogs?limit=4`;
    fetchRelated({ url }, handleResponse);
  }, [fetchRelated]);
                  ...
                  <div className="flex-1 space-y-10">
                    <div className="space-y-4">
                      <div className="flex items-center space-x-4 text-[10px] font-black text-[#8C0202] uppercase tracking-[0.3em]">
                        ...
                        <span className="text-gray-400">{post.profiles?.username || 'Admin'}</span>
                      </div>
        ...
        <section className="bg-gray-50 py-24 border-t border-gray-100">
          <div className="max-w-7xl mx-auto px-6">
            <ScrollReveal direction="bottom">
              <h3 className="text-3xl font-black text-gray-900 mb-12 tracking-tight">You May Also Like</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                {relatedPosts.slice(0, 4).map((post) => (
                  <Link key={post.id} to={`/posts/${post.id}`} className="group bg-white rounded-2xl overflow-hidden border border-gray-100 hover:shadow-xl transition-all hover:-translate-y-2">
                    <div className="aspect-[16/10] overflow-hidden">
                      <img
                        src={post.image_url || 'https://images.unsplash.com/photo-1499750310107-5fef28a66643?q=80&w=400'}
                        alt={post.title}
                    ...
                    <div className="p-6">
                      ...
                      <div className="flex items-center gap-4 mt-4 text-gray-400 text-xs">
                        <span className="flex items-center gap-1"><FaHeart className="text-[#8C0202]" /> {post.views || 0}</span>
                        <span className="flex items-center gap-1"><FaComment /> {post.comment_count || 0}</span>
                      </div>
                    </div>
                  </Link>
                ))}
