    ...
    return (
        <div className="space-y-6 animate-fade-in">
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-xl font-bold text-gray-900 mb-1">Comments</h1>
                    <p className="text-sm text-gray-500">Manage user feedback and engagement</p>
                </div>
                <div className="flex items-center space-x-2 bg-white px-4 py-2 rounded-md border border-gray-200/80">
                    <span className="text-[11px] font-bold text-gray-700">{comments.length}</span>
                                    ...
                                    <tr key={comment.id} className="group hover:bg-gray-50/80 transition-colors">
                                        <td className="py-4 px-6">
                                            <div className="flex items-center space-x-3">
                                                <div className="w-9 h-9 bg-gray-100 rounded-md flex items-center justify-center text-gray-500 border border-gray-200/80 uppercase font-bold text-[10px] shrink-0 overflow-hidden">
                                                ...
                                                <p className="text-[13px] font-semibold text-gray-800">{comment.profiles?.username || "Guest User"}</p>
                                            </div>
                                        </td>
                                        <td className="py-4 px-6 max-w-[280px]">
                                            <p className="text-[12px] text-gray-500 italic leading-relaxed line-clamp-2">"{comment.content}"</p>
                                        </td>
                                        <td className="py-4 px-6">
                                            <p className="text-[11px] font-medium text-gray-700 line-clamp-1 max-w-[180px]">{comment.blogs?.title || "Deleted Post"}</p>
                                        </td>
                                        <td className="py-4 px-6 whitespace-nowrap">
                                            <p className="text-[10px] font-medium text-gray-400 uppercase tracking-wide">{new Date(comment.created_at).toLocaleDateString()}</p>
                                        </td>
                                    ...
                                    <tr>
                                        <td colSpan="5" className="py-16 text-center">
                                            <div className="flex flex-col items-center space-y-3">
                                                <div className="w-14 h-14 bg-gray-100 rounded-lg flex items-center justify-center text-gray-300"><AiOutlineMessage size={28} /></div>
                                                <p className="text-[11px] font-bold text-gray-400 uppercase tracking-wider">No comments found</p>
                                            </div>
                                        </td>
                                    </tr>
                                )}
